...
LFCPAInstModel::extractToken(llvm::GetElementPtrInst *Inst) {
  ...
  if (isStructFieldPointerTy(Inst) or
      Inst->getOperand(0)->getType()->getContainedType(0)->isStructTy()) {
...
template <typename GOP> bool LFCPAInstModel::isStructFieldPointerTy(GOP *G) {
  ...
  /*	long int cntOp = G->getNumOperands();
          ...
          llvm::ConstantInt *CI =
     llvm::dyn_cast<llvm::ConstantInt>(G->getOperand(cntOp-1)); auto conVal =
     CI->getSExtValue(); return
     (BaseTy->getStructElementType(conVal)->isPointerTy());*/
}
