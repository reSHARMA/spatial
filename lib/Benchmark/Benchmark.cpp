...
std::string print_args(llvm::CallInst* ci){    
	...
	while(i<ci->getNumArgOperands())
	{
		...
		result += arg;			
		i++;
		if(i!=ci->getNumArgOperands()) result += ", " ;		
	}
    return result;
}

spatial::Soundness BenchmarkRunner::CheckSoundness(){
    for(auto i : Result){
...
std::ostream& operator<<(std::ostream& OS, const BenchmarkRunner& B) {
    ...
    OS << header + "\n";
    OS << "---------------------------------------------------------------------------------------------------------------------------\n\n";    
    for (auto R = B.Result.begin();R != B.Result.end();R++) {
        if(R == B.Result.begin()){
            OS << "\t\t=================================================================================\n";
        }
    ...
    if(B.SoundOrUnsound == Soundness::Sound){
        OS << "\nSoundness Check : Analysis is \033[1;92cmSound\033[0;m\n";
    }
    else{
...
void BenchmarkRunner::evaluatePrecision(llvm::Function& F){
        ...
        for(llvm::Instruction& I : BB){
            if(llvm::CallInst* ci = llvm::dyn_cast<llvm::CallInst>(&I)){
                if(ci->getCalledFunction()->getName().contains("DoesNotPointsTo")){
                    std::vector<std::string> res = Result[&I];
                    if(res[0] == "Fail"){
