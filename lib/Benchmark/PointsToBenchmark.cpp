...
std::string print_args(llvm::CallInst *CI) {
  ...
  while (i < CI->getNumArgOperands()) {
    std::string arg = llvm::dyn_cast<llvm::Instruction>(CI->getArgOperand(i))
                          ->getOperand(0)
                          ->getName()
                          .str();
    TCResult += arg;
...
std::ostream &operator<<(std::ostream &OS, const PointsToBenchmarkRunner &B) {
  ...
  for (auto R = B.TCResult.begin(); R != B.TCResult.end(); R++) {
    ...
    OS << std::setw(15) << "\t\t|  TestCase " << i << ":\t|\t" << std::setw(10)
       << "Line:" << (*R).second[2] << "   " << (*R).second[1] << "("
       << print_args(CI) << ")\t" << std::setw(5) << "|\t";
    res = (((*R).second[0] == "Pass") ? (GREENT + (*R).second[0] + RST)
                                      : (REDT + (*R).second[0] + RST));
    OS << res;
    OS << "\t|\n";
    OS << "\t\t================================================================"
  ...
  for (auto i : B.PrecisionResult) {
    ...
    OS << "\t\t|\tOut [" << (i.first)->getName().str() << "]\t" << std::setw(1)
       << "|";
    OS << std::setw(14) << "\t" << i.second[0] << "\t" << std::setw(13)
       << "\t|\t" << std::setw(14) << i.second[1] << std::setw(13) << "\t|\n";
    OS << "\t\t================================================================"
          "=========================================\n";
  }
...
PointsToBenchmarkRunner::extract(llvm::Instruction *Inst) {
    ...
    if (CalledFunction && CalledFunction->getName().contains(this->Key)) {
      for (llvm::Value *CallArg : CI->args()) {
        if (CallArg != *(CI->args().begin()))
          Tokens.push_back(
              llvm::dyn_cast<llvm::Instruction>(CallArg)->getOperand(0));
        else
    ...
    for (llvm::Instruction &I : BB) {
      if (llvm::CallInst *CI = llvm::dyn_cast<llvm::CallInst>(&I)) {
        if (CI->getCalledFunction()->getName().contains("DoesNotPointsTo")) {
          ...
          if (res[0] == "Fail") {
            // Format -- Pointer Count for which it fails, Total predicates it
            // fails;
            llvm::Value *arg1 =
                llvm::dyn_cast<llvm::Instruction>(CI->getArgOperand(0))
                    ->getOperand(0);
            if (!visited[arg1]) {
