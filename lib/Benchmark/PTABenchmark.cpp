...
spatial::Soundness PTABenchmarkRunner::CheckSoundness(){
    for(auto i : TCResult){
...
std::vector<llvm::Value*> PTABenchmarkRunner::extract(llvm::Instruction* Inst) {
    ...
    if (llvm::CallInst* CI = llvm::dyn_cast<llvm::CallInst>(Inst)) {        
        ...
        if (CalledFunction && CalledFunction->getName().contains("PointsTo")) {            
...
void PTABenchmarkRunner::evaluatePrecision(llvm::Function& F){
